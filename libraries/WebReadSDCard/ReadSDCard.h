...
const char readsdcard [] PROGMEM = R"rawliteral(
    <!DOCTYPE HTML>
    <html lang="en">
        <head>
        ...
        <body>
            <div class="container">
                <h1 align="center">-= OTA WEB FOR ESP32 =-</h1>
                ...
                <div id="OTAdetails" style="display:none;">
                    <input type="file" id="fileInput" onchange="analyzeFile()"
                        style="display:none;" accept=".bin">
                    <div id="analysisOutput"></div>
                    <button id="uploadApp" style="display:none;">Start
                        Update</button>
                    <button id="uploadSpiffs" style="display:none;">SPIFFS
                        Update</button>
                    <div id="spiffsInfo" style="display:none;">
                        <br>
                        <p>
                            <b>SPIFFS</b>: The SPIFFS (Serial Peripheral Interface
                            Flash File System) is a file system designed to be
                            used with NOR flash memory devices and microcontrollers
                            that support the Serial Peripheral Interface
                            (SPI) protocol such as the ESP32
                            <br>
                            <br>
                            Most of the apps over the internet don't need SPIFFS to
                            work, but there are some exceptions like <b>OrcaOne</b>.
                        </p>
                    </div>
                </div>
            ...
            <script>
                        ...
                        if (xmlhttp.status === 200) {
                            ...
                            lines.forEach(function (line) {
                                if (line) {
                                    ...
                                    var content = line.substring(3); // bỏ tiền tố "Fo " hoặc "Fi "
                                    if (type === "Fo") {
                                        foldersArray.push({ path: folder + content, name: content });
                                    } else if (type === "Fi") {
                                        var sepIndex = content.lastIndexOf(':');
                                        var filename = content.substring(0, sepIndex);
                                        var size = content.substring(sepIndex + 1).trim();
                                        filesArray.push({ path: folder + filename, name: filename, size: size });
                                    }
                                }
                            });
                ...
                function downloadDeleteButton(filename, action) {
                    ...
                    if (action === "create") {
                        ...
                        actualFolder = _("actualFolder").value;
                        if (actualFolder !== "" && !actualFolder.endsWith("/")) {
                            actualFolder += "/";
                        }
                        fullPath = actualFolder + filename;
                    }
